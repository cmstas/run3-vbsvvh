...
RNode applyQCDScoreResampling(RNode df, const std::string& run_number) {
    ...
    df = df.Define("_qcd_resampled_hv",
                   ...
                   {"isData", "shortname", "_qcd_resample_seed", "fatjet_pt", "fatjet_eta",
                    "fatjet_Hbb", "fatjet_Wqq"});

    // fatjet_Hbb / fatjet_Wqq are this branch's names for the quantities resampling.py
    ...
    df = df.Redefine("fatjet_Wqq", [](const RVec<RVec<float>>& hv) { return hv[1]; }, {"_qcd_resampled_hv"});
    return df;
}
