...
def main():
    parser = argparse.ArgumentParser(description="Run 0LEP 3FJ Analysis")
    ...
    presel = "boosted_h_candidate_score >= 0.1 & boosted_v1_candidate_score >= 0.1 & boosted_v2_candidate_score >= 0.1"
    ...
    for scan_idx in range(args.num_scans):
        ...
        sig, (cut_var1_best, cut_var2_best, cut_var3_best, cut_dnn_best, vbs_cut_best) = optimize_cuts(
            df_sig,
            df_bkg,
        ...
        if previous_cuts:
            ...
            for i, (prev_h, prev_v1, prev_v2) in enumerate(previous_cuts):
                print(f"    - NOT (boosted_h_candidate_score >= {prev_h:.4f} AND boosted_v1_candidate_score >= {prev_v1:.4f} AND boosted_v2_candidate_score >= {prev_v2:.4f})")
        ...
        orth_mask = ((df["boosted_h_candidate_score"] >= cut_var1_best)
                     & (df["boosted_v1_candidate_score"] >= cut_var2_best)
                     & (df["boosted_v2_candidate_score"] >= cut_var3_best))
        df = df[~orth_mask]
        if df_data is not None:
            data_orth_mask = ((df_data["boosted_h_candidate_score"] >= cut_var1_best)
                              & (df_data["boosted_v1_candidate_score"] >= cut_var2_best)
                              & (df_data["boosted_v2_candidate_score"] >= cut_var3_best))
            df_data = df_data[~data_orth_mask]

        previous_cuts.append((cut_var1_best, cut_var2_best, cut_var3_best))
        scan_cuts[f"Scan{scan_idx + 1}"] = {
            "h_cut": round(float(cut_var1_best), 4),
            "v1_cut": round(float(cut_var2_best), 4),
            "v2_cut": round(float(cut_var3_best), 4),
